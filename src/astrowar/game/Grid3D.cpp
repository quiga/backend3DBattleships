...
void Grid3D::handleInput(OIS::Keyboard* keyboard, OIS::Mouse* mouse, Ogre::Real dt)
{
	// Rotating the marker
...
bool Grid3D::queryResult(Ogre::MovableObject* obj, Ogre::Real distance)
{
	if (obj->getName() == mGridShape->getName())
	{
		...
		auto vec = mMouseRay.getPoint(distance);
...
void Grid3D::processSelected()
{
	if (mGridListener)
	{
		mGridListener->onSelect(mMarkerCoords[0], mMarkerCoords[1], mMarkerCoords[2]);
	}
}
